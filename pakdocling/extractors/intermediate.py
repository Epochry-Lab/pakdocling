"""Intermediate Certificate Extractor for BISE 12th grade / HSSC certificates."""
...
class IntermediateExtractor(BaseExtractor):
    ...
    def _extract_group(self, text: str) -> Union[str, None]:
        lower = text.lower()
        if "pre-engineering" in lower or "pre engineering" in lower:
            return "Pre-Engineering"
        if "pre-medical" in lower or "pre medical" in lower:
            return "Pre-Medical"
        if "ics" in lower or "computer science" in lower:
            return "ICS (Computer Science)"
        if "commerce" in lower or "i.com" in lower or "icom" in lower:
            return "Commerce"
        if "general science" in lower:
            return "General Science"
        if "humanities" in lower or "arts" in lower or "f.a" in lower:
            return "Humanities"
        return None
    ...
    def extract(self, items: list[OCRResultItem], raw_text: str) -> IntermediateCertificateData:
        ...
        if not total_marks and obtained_marks:
            total_marks = 1100.0  # Standard total marks for HSSC in Pakistan
